...
def build_input_specs(project_root: Path) -> list[InputFileSpec]:
    return [
        InputFileSpec(
            key="roads",
            label="背景道路網",
            path=project_root / "Data" / "app_init" / "roads" / "road_network.geojson",
            required=True,
        ...
        InputFileSpec(
            key="route",
            label="現実的ルート",
            path=project_root / "Data" / "app_runtime" / "routes" / "latest_route.geojson",
            required=True,
            description="再計算による出力ルートGeoJSON",
        ),
        InputFileSpec(
            key="priority_roads",
            label="優先度付き道路（任意）",
            path=project_root / "Data" / "app_init" / "roads" / "priority_roads_level3.geojson",
            required=False,
